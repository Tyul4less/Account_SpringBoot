...
<script>
	...
	function showApprovalStatusList() { // 거래처리스트 불러오기
		...
		$.ajax({
					type : "POST",
					url : "${pageContext.request.contextPath}/company/getAllWorkplaceListo",
					dataType : "json",
					success : function(jsonObj) {
						var  approvalStatusList= []; // 승인 리스트
						 console.log(jsonObj.slipList);
                         if (jsonObj.errorCode == 0) {
                             gridOptions.api.setRowData(jsonObj.allWorkplaceList);
                         } else {
                             alert("오류발생");
                         }
	...
	function updateApprovalStatus(getSelectedSell) {
		$.ajax({
					type : "POST",
					url : "${pageContext.request.contextPath}/company/updateApprovalStatus",
					data : {
	...
	function eliminationWorkplace(getSelectedSell) {
		$.ajax({
					type : "POST",
					url : "${pageContext.request.contextPath}/company/eliminationWorkplace",
					data : {
						"codes" : JSON.stringify(getSelectedSell)
					},
					dataType : "json",
    ...
    function showWorkplace(workplaceCode) { //slip rowid 선택한 전표행이다
        ...
        //gridOptions2.api.setPinnedBottomRowData();  
            ...
            $.ajax({
                type: "POST",
                async: false,
                url: "${pageContext.request.contextPath}/company/getWorkplaceo",
                data: {
                    "workplaceCode": workplaceCode
                },
                dataType: "json",
                success: function (jsonObj) {
				$('#workplaceCode').val(jsonObj.workplaceBean)
